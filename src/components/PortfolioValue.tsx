...
const PortfolioValue = ({ totalValue, gains, apy }: PortfolioValueProps) => {
  ...
  return (
    <div className="bg-[#0f0f0f] text-white rounded-lg w-full relative overflow-hidden">
      {/* Desktop Layout */}
      <div className="hidden md:block h-full min-h-[500px]">
        ...
        <div className="absolute inset-0 z-10">
          ...
          <div className="absolute bottom-0 left-0 right-0" style={{ height: '60%' }}>
            <PerformanceChart width={900} height={300} isMobile={false} />
          </div>
          </div>
        </div>
      </div>
      ...
      <div className="md:hidden p-4">
        {/* Vault title with dots indicator */}
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium text-white">Vault</h2>
        ...
        <div className="w-full h-32 relative">
          <PerformanceChart width={320} height={128} isMobile={true} />
        </div>
      </div>
    </div>
  );
